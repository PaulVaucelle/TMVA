...
void doTraining(){
    ...
    dataloader->AddVariable( "mva_track_firstHit_x", 'F' );
    dataloader->AddVariable( "mva_track_firstHit_y", 'F' );
    dataloader->AddVariable( "mva_track_firstHit_z", 'F' );
    ...
    dataloader->AddVariable( "mva_track_nhits", 'F' );
    ...
    dataloader->AddVariable( "mva_ValTIBHit", 'F');
    dataloader->AddVariable( "mva_ValTOBHit", 'F');
    dataloader->AddVariable( "mva_ValPixBarHit", 'F');
    ...
    //Paul//
    factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG",
         "!H:!V:NTrees=1000:MinNodeSize=2.5%:MaxDepth=3:BoostType=Grad:UseBaggedBoost=False:GradBaggingFraction=0.6:Shrinkage=0.1:SeparationType=GiniIndex:nCuts=20:UseYesNoLeaf=True:UseRandomisedTrees=True");//Possible overtraining
            //////Parameter tested independantly: (with S= 10 000 and bkg =10000)
                        // Best ROC obtained is 0.885 (20000,20000, NTrees=1000:MinNodeSize=2.5%:MaxDepth=5:BoostType=Grad:UseBaggedBoost=False:GradBaggingFraction=0.6:Shrinkage=0.1:SeparationType=GiniIndex:nCuts=20:UseYesNoLeaf=True:UseRandomisedTrees=True)
                        // 
