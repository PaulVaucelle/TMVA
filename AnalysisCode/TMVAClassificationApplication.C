...
// --- End of event loop: Real time 0:00:01, CP time 1.360
...
//    }
   ...
   // Book method(s): changeable if i only use two methods
   for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
      if (it->second) {
...
//    }
   ...
   TTree* theTree = (TTree*)input->Get("Signal");//TreeS
   Float_t userVar1, userVar2;
   theTree->SetBranchAddress( "mva_track_pt", &mva_track_pt );
   theTree->SetBranchAddress( "mva_track_eta", &mva_track_eta );
   theTree->SetBranchAddress( "mva_track_nchi2", &mva_track_nchi2);
   theTree->SetBranchAddress( "mva_track_nhits", &mva_track_nhits );
   theTree->SetBranchAddress( "mva_track_algo", &mva_track_algo );
   theTree->SetBranchAddress( "mva_ntrk10", &mva_ntrk10 );
   theTree->SetBranchAddress( "mva_drSig", &mva_drSig );
   theTree->SetBranchAddress( "mva_track_isinjet", &mva_track_isinjet);
...
//    if (Use["Fisher"]) { if (probHistFi != 0) probHistFi->Write(); if (rarityHistFi != 0) rarityHistFi->Write(); }
   Double_t BDTvalue = reader->EvaluateMVA( "BDT method", effS );
   Double_t BDTGvalue = reader->EvaluateMVA( "BDTG method", effS );
   std::cout << "---------- BDT value :"<< BDTvalue << std::endl;
   std::cout << "---------- BDTG value :"<< BDTGvalue << std::endl;
   target->Close();
